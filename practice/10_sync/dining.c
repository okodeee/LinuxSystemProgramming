...
DiningPhilosopher(int *pId)
{
	...
	for (i = 0 ; i < NLOOPS ; i++)  {
		...
		if (sem_wait(&ChopStick[id]) < 0)  { // 왼쪽 젓가락 들기
			perror("sem_wait");
			pthread_exit(NULL);
		}
		if (sem_wait(&ChopStick[(id+1) % NUM_MEN]) < 0)  { // 오른쪽
			perror("sem_wait");
			pthread_exit(NULL);
		}
		...
		if (sem_post(&ChopStick[id]) < 0)  { // 왼쪽 젓가락 내려놓기
			perror("sem_post");
			pthread_exit(NULL);
		}
		if (sem_post(&ChopStick[(id+1) % NUM_MEN]) < 0)  { // 오른쪽
			perror("sem_post");
			pthread_exit(NULL);
		}
	}
...
main()
{
	...
	for (i = 0 ; i < NUM_MEN ; i++)  { // 젓가락 별로 초기화
		if (sem_init(&ChopStick[i], 0, 1) < 0)  { // 초기값 1로 설정
			perror("sem_init");
			exit(1);
		}
		id[i] = i;
