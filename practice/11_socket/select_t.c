...
ProcessTcpRequest()
{
	...
	msg.type = MSG_REPLY;
	sprintf(msg.data, "This is a reply from pid: %d, tid: %ld.", getpid(), pthread_self());
	...
	while (1)  {
		...
		while (count--)  {
			if (FD_ISSET(TcpSockfd, &fdvar))  { // 어떤 소켓에서 왔는지 확인
				...
				/* creating a thread and then immediately joining it,
				 * waiting for it to finish. This causes the threads
				 * to be created sequentially rather than in parallel,
				 * allowing their IDs to be recycled.
				 */
			}
